...
import { bagOutline, checkmark, heartDislike, heartOutline, reloadOutline } from "ionicons/icons";
...
function ProductDetails() {
  ...
  const { addProductToWishList, wishListed, removeProductFromWishList } = useWishList();
  ...
  const { data, isLoading, error, refetch } = useQuery(
    ...
    () => (shopId && shopTitle ? CategoryServiceItem(shopId, shopTitle) : getFeaturedProductService(featId, featTitle)),
    ...
    if (isLoading) {
      ...
      return (
        <div className="flex justify-center">
          <Icon icon={reloadOutline} style={{ fontSize: "7rem" }} actionButton={() => refetch()} />
        </div>
      );
    } else {
      return (
            ...
            <div className="grid grid-cols-2 gap-x-20">
              <div className="grid grid-cols-2 w-full h-full gap-x-1 gap-y-4">
                {data.images?.map((image) => (
                  <img key={image} loading="lazy" src={image} alt={image} className="w-full h-full" />
                ))}
              </div>
            </div>
              ...
              <div className="mt-4 lg:row-span-3 lg:mt-0">
                ...
                <div className="mt-6">
                  <div className="flex items-center">
                    <div className="flex items-center">{PRODUCT_RATINGS(data.rating)}</div>
                  </div>
                </div>
                <div className="grid mt-8 items-center text-center justify-center">
                  ...
                  >
                    <Icon style={{ fontSize: "1.5rem", color: "white", marginRight: "1rem" }} icon={productAdded ? checkmark : bagOutline} />
                  ...
                  >
                    <Icon style={{ fontSize: "1.5rem", color: "white", marginRight: ".7rem" }} icon={wishList ? heartDislike : heartOutline} />
                    {wishList ? `${t("home.disLike")}` : `${t("home.addToWishlist")}`}
                  </button>
                </div>
              </div>
              <div className="mt-4">
                <p>{data.stock} {t("available")}</p>
              </div>
            </div>
          </div>
          <div className="mx-auto px-8">
            <h1 className="font-medium tracking-wide uppercase">{t("customerPurchase")}</h1>
          </div>
        </div>
      );
    }
